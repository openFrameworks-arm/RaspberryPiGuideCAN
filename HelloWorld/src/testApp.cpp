...
void testApp::update()
{
	...
	float texCoordStepU = (frontBuffer->getTextureReference().getTextureData().tex_t) / (float)(resX-1);
	float texCoordStepV = (frontBuffer->getTextureReference().getTextureData().tex_u) / (float)(resY-1);
...
void testApp::draw()
{
	...
	ofEnableAlphaBlending();
		frontBuffer->begin();
			...
			ofSetColor( ofColor::fromHsb( (int)ofMap( sinf(ofGetElapsedTimef()/6.0f), -1.0f, 1.0f, 0.0f, 255.0f) , 255, 200 ) ); // get ourselves a colour based on rotating the hue
